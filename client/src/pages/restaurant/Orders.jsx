...
import { RestaurantGetOrders, RestaurantUpdateOrderStatus } from "../../api";
...
const Orders = () => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await RestaurantGetOrders(token);
      setOrders(res.data || []);
    } catch (err) {
      dispatch(
        openSnackbar({
  ...
  const onChangeStatus = async (orderId, status) => {
    if (!token) return;
    try {
      const res = await RestaurantUpdateOrderStatus(token, orderId, status);
